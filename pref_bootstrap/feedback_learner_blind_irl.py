...
from pref_bootstrap.algos.mce_irl import (mce_partition_fh,
...
class BlindIRLFeedbackModel(EnvFeedbackModel):
    ...
    def log_likelihood(self, data, reward_model, bias_params):
        ...
        log_trans_probs = jnp.mean(jnp.sum(jnp.log(trans_probs), axis=1),
                                   axis=0)
    ...
    def _ll_compute_oms(self, data, reward_model, bias_params):
        ...
        empirical_om_t = jnp.mean(empirical_om_flat.reshape(om_t_shape),
                                  axis=0)
