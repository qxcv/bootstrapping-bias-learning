...
class FixedGaussianPrior(Prior):
    ...
    def sample(self, key):
        ...
        vec = jrandom.normal(
            inner_key, shape=self.shape) * self.std + self.mean
        return key, vec
...
class LogNormalPrior(Prior):
    ...
    def log_prior(self, params):
        return -jnp.log(params - self.mu)**2 / (
            2 * self.std**2) - params * self.std * jnp.sqrt(2 * jnp.pi)
    ...
    def sample(self, key):
        key, inner_key = jrandom.split(key)
        vec = jnp.exp(
            jrandom.normal(key, shape=self.shape) * self.std + self.mu)
        return key, vec
...
class BetaPrior(Prior):
    ...
    def log_prior(self, params):
        nelem = params.size
        gamma_term = nelem * (
            jsp.special.gammaln(self.alpha) + jsp.special.gammaln(self.beta) -
    ...
    def sample(self, key):
        key, inner_key = jrandom.split(key)
        vec = jrandom.beta(
            key=key, a=self.alpha, b=self.beta, shape=self.shape)
        return key, vec
