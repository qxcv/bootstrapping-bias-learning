...
def run_agent(agent, env, episode_length=float("inf"), determinism=False):
    ...
    episode_length: The maximum number of actions that the agent can take. If
        the agent has not reached a terminal state by this point, the episode
        is terminated early.

    Returns the trajectory that the agent took, which is a list of (s, a, s',
    r) tuples.
    """
...
def evaluate_proxy(walls,
    ...
    True reward is summed if the reward grid's entry at the given state can be
    cast to a float
