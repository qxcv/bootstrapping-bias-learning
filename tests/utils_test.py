...
class TestDistribution(unittest.TestCase):
    def test_equality(self):
        self.assertEqual(
            Distribution({"a": 0.5, "b": 0.5}), Distribution({"a": 1, "b": 1})
        )
        self.assertEqual(
            Distribution({"a": 0.5, "b": 0.5, "c": 0}),
            Distribution({"a": 0.5, "b": 0.5, "d": 0}),
        )
        self.assertNotEqual(Distribution({"a": 1, "b": 1}), Distribution({"a": 1}))
    ...
    def test_as_numpy_array(self):
        ...
        self.almostEqual(list(dist.as_numpy_array(fn, 5)), [0.2, 0.4, 0, 0.4, 0])
